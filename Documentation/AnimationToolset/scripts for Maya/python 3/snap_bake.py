...
def add_scale_key_at_first_frame():
    ...
    try:
        ...
        flip_jnt_list = dict((v, k) for k, v in list(name_len.items()))
        top_jnt = list(flip_jnt_list.values())[0]
...
def facial_animation_dict():
    ...
    for key, value in list(shape_list.items()):
        try:
    ...
    for key, frame_value in list(key_anim_temp.items()):
    ...
    for key_k, value_k in list(key_anim_dict.items()):
...
def save_fbx_lens_console(file_path):
    ...
    if cmds.objExists("*:PROP"):
        ...
        dup_list = [list(itm) for itm in list(dict_tuple.keys())]
        set_dup = []
